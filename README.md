...
* **gitignore**: Plain text in `.gitignore`, generated based on **ignores**. For each package in **ignores**, the `bdgi git` command takes elements from `ignore` Array and `unignore` Array by turns and prefix bower_components and package name to them.
