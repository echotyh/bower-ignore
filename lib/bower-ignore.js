...
function composeAndSet(bowerJSON, force, update) {
  var res = compose.conposeBowerIgnore(bowerJSON, force, update);
...
exports.runJson = function(download, force, debug) {
  try {
    ...
    if (download) {
      fetch.fetchBowerIgnore(bowerJSON, function (update) {
        composeAndSet(bowerJSON, force, update);
      });
    }
    else {
