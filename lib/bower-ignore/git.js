...
exports.setGitignore = function(gitIgnore) {
  if (!gitIgnore || (gitIgnore instanceof Array && gitIgnore.length == 0)) {
    return ;
  }
  ...
  if (fs.existsSync(GITIGNORE)) {
    ...
    ignoreOut = halves ? join(halves.firsHalf, gitIgnore, halves.lastHalf) :
                         join(ignoreIn, TIP, BEGIN, gitIgnore, END, '');
  }
  else {
    ignoreOut =  join(TIP, BEGIN, gitIgnore, END, '');
  }
